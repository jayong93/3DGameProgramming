...
CWireCubeMesh::CWireCubeMesh(ID3D11Device * device, float width, float height, float depth)
{
	vertexCnt = 36;
	strideByte = sizeof(CDiffusedVertex);
	offset = 0;
...
CWireCubeMesh::~CWireCubeMesh()
{
}

void CWireCubeMesh::CreateRasterizerState(ID3D11Device * device)
{
	D3D11_RASTERIZER_DESC rd;
	ZeroMemory(&rd, sizeof(rd));
	rd.CullMode = D3D11_CULL_NONE;
...
void CWireCubeMesh::Render(ID3D11DeviceContext * deviceContext)
{
	CMesh::Render(deviceContext);
}
